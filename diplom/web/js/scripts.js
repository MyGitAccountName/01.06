...
/*        $.ajax({
    ...
    $('.toPrev').click(function () {
        ...
        /*          let name = 'John';
              ...
              $.ajax({
                    url: '/index.php?r=test/testing.php',
                    type: 'POST',
                    dataType: 'json',
                    data: { name: name,
                            age: age,
...
/*        let cur = $('.questionField:visible');
   ...
   // $('#subjectTop').addEventListener('click', (event) => {
    ...
    $('.contents_item_title_inner').on('click',function(){
        if (!$(this).hasClass('show_all')) { $(this).parents('.contents_item').find('.contents_item_body').slideToggle(300);}
        $(this).toggleClass('open');
        if ($(this).hasClass('show_all')){
            if ($(this).hasClass('open')) {
                $(this).html('Свернуть все');

            //    $(this).parents('.contents_item').find('.contents_item_title_inner:not(.open)').trigger('click');
                $(this).parents('.contents_item').find('.contents_item_title_inner_level2:not(.open)').trigger('click');

      /*          $('.contents_item_title_inner:not(.open)').trigger('click');
                $('.contents_item_title_inner_level2:not(.open)').trigger('click');*/
            } else {
                $(this).html('Показать все');
/*                $('.contents_item_title_inner.open').trigger('click');
                $('.contents_item_title_inner_level2.open').trigger('click');*/


            //    $(this).parents('.contents_item').find('.contents_item_title_inner.open').trigger('click');
                $(this).parents('.contents_item').find('.contents_item_title_inner_level2.open').trigger('click');
            }
        }
    });

    $('.contents_item_title_inner_level2').on('click',function(){
        $(this).parents('.contents_item_level2').find('.contents_item_body_level2').slideToggle(300);
        $(this).toggleClass('open');
/*        if ($(this).hasClass('show_all')){
            if ($(this).hasClass('open')) {
                $(this).html('Свернуть все');
