...
<!--    --><?/*= $form->field($model, 'theme1')->dropDownList(ArrayHelper::map(Theme::find()->where("subject=1")->all(), 'id', 'subtheme'),
    ...
    <?= $form->field($model, 'theme1')->dropDownList(ArrayHelper::map(SubTheme::find()->all(), 'id', 'subtheme'),
        ['id' => 'qTheme1Field', 'class' => 'form-control col-lg-5']); ?>

    <?= $form->field($model, 'theme2')->dropDownList(ArrayHelper::map(SubTheme::find()->where("subject=1")->all(), 'id', 'subtheme'),
