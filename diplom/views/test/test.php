...
<div class="site-test">
    ...
    echo '<a class="testSubject" href="'.Yii::$app->urlManager->createUrl(['site/subject', 'id' => $subject['sKey']]).'">'. $subject['sName'] ."</a><br>";
endforeach;
...
//var_dump($questionList);
    ...
    <?= Yii::$app->user->isGuest ? "<span class='alert'>Для прохождения теста необходима <a href=".Yii::$app->urlManager->createUrl(['site/login']).">авторизация</a>!</span>" :
        "<a class = 'button' href=".Yii::$app->urlManager->createUrl(['test/testing', 'testID' => $id, /*'testName' => $name, 'curQuestion' => 0,*/ 'time' => $time]).">Начать</a>"?>
